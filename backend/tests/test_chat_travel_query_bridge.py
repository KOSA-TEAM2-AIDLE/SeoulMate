...
class ChatTravelQueryBridgeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_ready_query_resolves_explicit_location_into_existing_chat_fields(self):
        service = _FakeTravelQueryService(_ready_response())
        resolver = _FakeLocationResolver(ResolvedSearchLocation(
            location_name="서울시청", latitude=37.5663, longitude=126.9779, source="kakao_local",
        ))

        resolved, response = await resolve_travel_query(
            ChatRequest(message="서울 전시 추천", lang="ko"), service=service,
            location_resolver=resolver,
        )

        self.assertIsNone(response)
        self.assertEqual((37.5663, 126.9779, "서울시청"), (resolved.lat, resolved.lng, resolved.location_name))
        self.assertEqual("서울", resolver.calls[0]["location"])

    async def test_ambiguous_location_uses_same_thread_for_numbered_choice(self):
        service = _FakeTravelQueryService(_ready_response())
        candidates = [
            ResolvedPlace(place_id="1", place_name="서울문화센터 본관", latitude=37.5, longitude=127.0),
            ResolvedPlace(place_id="2", place_name="서울문화센터 별관", latitude=37.6, longitude=127.1),
        ]
        resolver = _FakeLocationResolver(ResolvedSearchLocation(
            location_name="서울", candidates=candidates, requires_disambiguation=True,
        ))

        unresolved, collecting = await resolve_travel_query(
            ChatRequest(message="서울 전시 추천", lang="ko"), service=service,
            location_resolver=resolver,
        )
        resumed, response = await resolve_travel_query(
            ChatRequest(message="2번", lang="ko", travel_query_thread_id=collecting.thread_id),
            service=service, location_resolver=resolver,
        )

        self.assertIsNone(unresolved)
        self.assertEqual("collecting", collecting.status)
        self.assertIn("1.", collecting.assistant_message)
        self.assertIsNone(response)
        self.assertEqual((37.6, 127.1), (resumed.lat, resumed.lng))
        self.assertEqual([], service.resumed)
