...
BACKEND = Path(r"C:\Users\user\Desktop\seoulmate\SeoulMate\backend")
OUTPUT = BACKEND / "notebooks" / "SeoulMate_rag_mcp_debug.ipynb"
...
cells = []
cells.append(nbf.v4.new_markdown_cell("""# SeoulMate RAG + Weather MCP 디버그 노트북
...
cells.append(nbf.v4.new_code_cell(r'''# 1. 절대 경로 기준 실행 환경
import os
import sys
...
cells.append(nbf.v4.new_markdown_cell("""## 2. 테스트 질문과 고정 Structured Query

실서비스에서는 상위 GPT가 이 JSON을 만듭니다. 여기서는 GPT 파싱 결과의 변동과 비용을 분리하기 위해 JSON을 직접 넣습니다."""))

cells.append(nbf.v4.new_code_cell(r'''# 이 셀을 수정해서 여러 질문을 테스트하세요.
QUESTION_CASE = {
    "language": "ko",
...
cells.append(nbf.v4.new_code_cell(r'''# 3. Structured Query -> 실제 Restaurant Search Plan
...
cells.append(nbf.v4.new_markdown_cell("""### RAG가 실제 사용하는 값
...
cells.append(nbf.v4.new_code_cell(r'''# 4. RAG 실행 전 입력 요약
rag_input = {
...
cells.append(nbf.v4.new_code_cell(r'''# 5. 원본 RAG 검색 실행 (MCP 재랭킹 전)
...
cells.append(nbf.v4.new_code_cell(r'''# 6. MCP 적용 전 전체 RAG 순위와 점수 구성
...
cells.append(nbf.v4.new_code_cell(r'''# 7. 후보별 리뷰/메뉴 근거를 행 단위로 펼쳐서 확인
evidence_rows = []
...
cells.append(nbf.v4.new_code_cell(r'''# 8. Weather MCP 원본 응답
...
cells.append(nbf.v4.new_code_cell(r'''# 9. Weather MCP 적용 후 전체 순위와 변경 이유
...
cells.append(nbf.v4.new_code_cell(r'''# 10. 실제로 순위가 바뀐 후보와 날씨 이유만 보기
changed_rows = [
...
cells.append(nbf.v4.new_code_cell(r'''# 11. 합성 날씨 시나리오 회귀 디버깅
...
cells.append(nbf.v4.new_code_cell(r'''# 12. 특정 후보의 모든 원본 값을 상세 확인
...
cells.append(nbf.v4.new_code_cell(r'''# 13. RAG_ONLY 안전성 확인: 날씨 조회/재랭킹 없이 원래 순서가 유지되어야 함
...
cells.append(nbf.v4.new_code_cell(r'''# 14. 선택 사항: 최종 GPT 답변까지 생성
...
cells.append(nbf.v4.new_code_cell(r'''# 15. 선택 사항: 이번 디버그 결과를 JSON으로 저장
SAVE_TRACE = False
